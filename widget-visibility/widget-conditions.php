...
class Jetpack_Widget_Conditions {
	...
	public static function widget_conditions_options_echo( $major = '', $minor = '' ) {
		switch ( $major ) {
			case 'category':
				?>
				<option value=""><?php _e( 'All category pages', 'jetpack-widget-visibility' ); ?></option>
				<?php
			...
			case 'author':
				?>
				<option value=""><?php _e( 'All author pages', 'jetpack-widget-visibility' );  ?></option>
				<?php
			...
			case 'tag':
				?>
				<option value=""><?php _e( 'All tag pages', 'jetpack-widget-visibility' );  ?></option>
				<?php
			...
			case 'date':
				?>
				<option value="" <?php selected( '', $minor ); ?>><?php _e( 'All date archives', 'jetpack-widget-visibility' );  ?></option>
				<option value="day"<?php selected( 'day', $minor ); ?>><?php _e( 'Daily archives', 'jetpack-widget-visibility' );  ?></option>
				<option value="month"<?php selected( 'month', $minor ); ?>><?php _e( 'Monthly archives', 'jetpack-widget-visibility' );  ?></option>
				<option value="year"<?php selected( 'year', $minor ); ?>><?php _e( 'Yearly archives', 'jetpack-widget-visibility' );  ?></option>
				<?php
			break;
			case 'page':
			case 'parent-page':
				...
				if ( $major != 'parent-page' ) {
					?>
				<option value="front" <?php selected( 'front', $minor ); ?>><?php _e( 'Front page', 'jetpack-widget-visibility' );  ?></option>
				<option value="posts" <?php selected( 'posts', $minor ); ?>><?php _e( 'Posts page', 'jetpack-widget-visibility' );  ?></option>
				<option value="archive" <?php selected( 'archive', $minor ); ?>><?php _e( 'Archive page', 'jetpack-widget-visibility' );  ?></option>
				<option value="404" <?php selected( '404', $minor ); ?>><?php _e( '404 error page', 'jetpack-widget-visibility' );  ?></option>
				<option value="search" <?php selected( 'search', $minor ); ?>><?php _e( 'Search results', 'jetpack-widget-visibility' );  ?></option>
				<optgroup label="<?php esc_attr_e( 'Post type:', 'jetpack-widget-visibility' );  ?>">
						<?php
				...
				?>
				<optgroup label="<?php esc_attr_e( 'Static page:', 'jetpack-widget-visibility' );  ?>">
			...
			case 'taxonomy':
				?>
				<option value=""><?php _e( 'All taxonomy pages', 'jetpack-widget-visibility' );  ?></option>
				<?php
				...
				foreach ( $taxonomies as $taxonomy ) {
					?>
					<optgroup label="<?php esc_attr_e( $taxonomy->labels->name . ':', 'jetpack-widget-visibility' );  ?>">
	...
	public static function widget_conditions_admin( $widget, $return, $instance ) {
		...
		<div class="widget-conditional <?php if ( empty( $_POST['widget-conditions-visible'] ) || $_POST['widget-conditions-visible'] == '0' ) { ?>widget-conditional-hide<?php } ?>">
			...
			<?php if ( ! isset( $_POST['widget-conditions-visible'] ) ) { ?><a href="#" class="button display-options"><?php _e( 'Visibility', 'jetpack-widget-visibility' ); ?></a><?php }  ?>
			<div class="widget-conditional-inner">
				<div class="condition-top">
					<?php printf( _x( '%s if:', 'placeholder: dropdown menu to select widget visibility; hide if or show if', 'jetpack-widget-visibility' ), '<select name="conditions[action]"><option value="show" ' . selected( $conditions['action'], 'show', false ) . '>' . esc_html_x( 'Show', 'Used in the "%s if:" translation for the widget visibility dropdown', 'jetpack-widget-visibility' ) . '</option><option value="hide" ' . selected( $conditions['action'], 'hide', false ) . '>' . esc_html_x( 'Hide', 'Used in the "%s if:" translation for the widget visibility dropdown', 'jetpack-widget-visibility' ) . '</option></select>' );  ?>
				</div><!-- .condition-top -->
						...
						<div class="condition">
							<div class="alignleft">
								<select class="conditions-rule-major" name="conditions[rules_major][]">
									<option value="" <?php selected( "", $rule['major'] ); ?>><?php echo esc_html_x( '-- Select --', 'Used as the default option in a dropdown list', 'jetpack-widget-visibility' );  ?></option>
									<option value="category" <?php selected( "category", $rule['major'] ); ?>><?php esc_html_e( 'Category', 'jetpack-widget-visibility' );  ?></option>
									<option value="author" <?php selected( "author", $rule['major'] ); ?>><?php echo esc_html_x( 'Author', 'Noun, as in: "The author of this post is..."', 'jetpack-widget-visibility' );  ?></option>
									<option value="tag" <?php selected( "tag", $rule['major'] ); ?>><?php echo esc_html_x( 'Tag', 'Noun, as in: "This post has one tag."', 'jetpack-widget-visibility' );  ?></option>
									<option value="date" <?php selected( "date", $rule['major'] ); ?>><?php echo esc_html_x( 'Date', 'Noun, as in: "This page is a date archive."', 'jetpack-widget-visibility' );  ?></option>
									<option value="page" <?php selected( "page", $rule['major'] ); ?>><?php echo esc_html_x( 'Page', 'Example: The user is looking at a page, not a post.', 'jetpack-widget-visibility' );  ?></option>
									<option value="parent-page" <?php selected( "parent-page", $rule['major'] ); ?>><?php echo esc_html_x( 'Page (include children)', 'Example: The user is looking at a page or child of a page.', 'jetpack-wideget-visibility' ); ?></option>
									<option value="taxonomy" <?php selected( "taxonomy", $rule['major'] ); ?>><?php echo esc_html_x( 'Taxonomy', 'Noun, as in: "This post has one taxonomy."', 'jetpack-widget-visibility' );  ?></option>
								</select>
								<?php _ex( 'is', 'Widget Visibility: {Rule Major [Page]} is {Rule Minor [Search results]}', 'jetpack-widget-visibility' );  ?>
								<select class="conditions-rule-minor" name="conditions[rules_minor][]" <?php if ( ! $rule['major'] ) { ?> disabled="disabled"<?php } ?> data-loading-text="<?php esc_attr_e( 'Loading...', 'jetpack-widget-visibility' );  ?>">
									<?php self::widget_conditions_options_echo( $rule['major'], $rule['minor'] ); ?>
								</select>
								<span class="condition-conjunction"><?php echo esc_html_x( 'or', 'Shown between widget visibility conditions.', 'jetpack-widget-visibility' );  ?></span>
							</div>
							<div class="condition-control alignright">
								<a href="#" class="delete-condition"><?php esc_html_e( 'Delete', 'jetpack-widget-visibility' );  ?></a> | <a href="#" class="add-condition"><?php esc_html_e( 'Add', 'jetpack-widget-visibility' );  ?></a>
							</div>
							<br class="clear" />
						</div><!-- .condition -->
						<?php
